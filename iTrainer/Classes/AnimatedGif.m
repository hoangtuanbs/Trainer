...
- (id) init
{
	...
	GIF_transparancies = [[NSMutableArray alloc] init];
...
- (NSArray*) getAnimatingImages {
	if ([GIF_framesData count] > 0)
	{
		...
		// Add all subframes to the animation
		NSMutableArray *array = [[NSMutableArray array] init];
		//UIImage * maskImage = [self getFrameAsImageAtIndex:0];
		UIImage *temp;
		[array addObject:[self getFrameAsImageAtIndex:0]];
		for (int i =1; i < [GIF_framesData count]; i++)
		{		
			temp = [self getFrameAsImageAtIndex:i];
			//UIImage *image = [self maskFrame:maskImage withFrame: temp];
			//maskImage = [self maskFrame:temp withFrame:maskImage];
	...
	for ( ; ; ) {
		...
		if (cur[0] == 0x04 && prev[0] == 0xF9)
		{
			...
			if (GIF_frameHeader == nil)
			{
				...
				GIF_frameHeader = [NSMutableData dataWithBytesNoCopy:board length:8];
			}
			break;
		}
...
{	
	[self GIFGetBytes:9];
	NSMutableData *GIF_screenTmp = [[NSMutableData alloc] initWithData:GIF_buffer];
	...
	[GIF_string setData:[[NSString stringWithString:@"GIF89a"] dataUsingEncoding: NSUTF8StringEncoding]];
...
- (UIImage *)maskFrame:(UIImage *)firstImage withFrame:(UIImage *)maskImage{	
	CGImageRef image = firstImage.CGImage;
	CGImageRef maskImageRef = maskImage.CGImage;
	CGRect bounds = CGRectMake(0, 0, CGImageGetWidth(image), CGImageGetHeight(image)); // clip 2 pixels hor and vert
	...
	// draw first image
	CGContextDrawImage(UIGraphicsGetCurrentContext(), CGRectMake(0, 0, CGImageGetWidth(image), CGImageGetHeight(image)), image);
	
	// draw mask
	//	CGContextDrawImage(UIGraphicsGetCurrentContext(), CGRectMake(0, 0, CGImageGetWidth(maskedImage), CGImageGetHeight(maskedImage)), maskedImage);
	
	const float myMaskingColors[6] = {0, 0, 0, 0, 0, 0};
	CGImageRef myColorMaskedImage = CGImageCreateWithMaskingColors (maskImageRef, myMaskingColors);
	CGContextDrawImage (UIGraphicsGetCurrentContext(), CGRectMake(0, 0, CGImageGetWidth(myColorMaskedImage), CGImageGetHeight(myColorMaskedImage)), myColorMaskedImage);	
	//CGContextDrawImage(UIGraphicsGetCurrentContext(), CGRectMake(0, 0, CGImageGetWidth(maskImageRef), CGImageGetHeight(maskImageRef)), maskImageRef);
	
	// rotate context back
	CGContextTranslateCTM(UIGraphicsGetCurrentContext(), 0, CGImageGetHeight(image));
	CGContextScaleCTM(UIGraphicsGetCurrentContext(), 1.0, -1.0);
